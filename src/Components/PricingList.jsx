...
//the property even will be applied to the even elements and the property odd will be applied to the odd elements
    ...
    <div className="flex gap-[1rem] max-lg:flex-wrap">
     {pricing.map((item,index)=>(
        <div key={item.id} className="w-[19rem] max-lg:w-full h-full px-6 bg-n-8 border  border-n-6 rounded-[2rem] lg:w-auto even:py-14 odd:py-8 odd:my-4 [&>h4]:first:text-color-2  [&>h4]:even:text-color-1 [&>h4]:last:text-color-3">
           ...
           <Button className="w-full mb-6" href={item.price?'/pricing':'mailto:contact@jsm.pro'}
            white={!!item.price}
           >
